...
\email{theo@henri-45.fr}                               % optional, remove / comment the line if not wanted
...
\cventry{2012--2014}{Université de Poitiers}{Master \og{}méthodes d'analyse du social\fg{} (option ACCES)}{}{}{(Sociologie, anthropologie, statistiques.)
	...
	\end{itemize}
	\textbf{Mémoire de master 1} : \emph{Wikipédia : une utopie réalisée ?} (86~pages). Sous la direction de Ludovic~\textsc{Gaussot} (jury~:~Mathias~\textsc{Millet}). Mention bien\\
	\textbf{Mémoire de master 2} : \emph{fr.wikipedia, un espace d'existence et de reconnaissance} (\emph{titre temporaire}). Sous la direction de Ludovic~\textsc{Gaussot}.
}
...
\hspace{.5em}{\small \cvitem{Traduction}{Participation à des sessions de traduction collaborative d'articles de l'anglais vers le français pour le blog \emph{\url{http://framablog.org}}.}}
...
\cvitem{Développement web}{HTML/CSS, PHP/SQL.}
\cvitem{Graphisme}{\textsc{Gimp}, Inkscape, Blender, Darktable.}
\cvitem{Administration de sites}{Drupal, Wordpress, \textsc{Spip}, CMSMadeSimple, MediaWiki, Dokuwiki.}
